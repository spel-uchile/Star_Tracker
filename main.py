...
# 2.- Define directorios y nombres básicos a usar.
# 3.- Lectura de la imagen que toma la camara.
# 4.- Ejecuta 'SExtractor' sobre la imagen.
# 5.- Prueba 'match' entre el catalogo de 'SExtractor' y todo el cielo (Primera iteración).
# 6.- A partir del resultado del mejor 'match', deproyecta y obtiene las coordenadas de cielo del punto central de la cámara.
# 7.- Busca las estrellas con las que se hizo 'match', obtiene su posicion (en coordenadas RA/DEC) y su magnitud.
# 8.- Proyecta las estrellas encontradas en (7.-) en el punto de cielo encontrado en (6.-).
# 9.- Prueba 'match' entre el catalogo entregado por 'SExtractor' y la nueva proyeccion obtenida en (8.-) (Segunda iteración).
# 10.- A partir del resultado de 'match', deproyecta y obtiene las coordenadas de cielo del punto central de la cámara.
# 11.- Vuelve a proyectar las estrellas encontradas en (7.-), ahora en el punto de cielo encontrado en (10.-).
# 12.- Prueba 'match' entre el catalogo entregado por 'SExtractor' y la nueva proyeccion obtenida en (11.-) (Tercera iteración).
# 13.- A partir del resultado de 'match', deproyecta y obtiene las coordenadas de cielo del punto central de la cámara.
# 14.- Imprime el resultado en pantalla, de las coordenadas de apuntamiento RA, DEC y Roll.

##---------------------//----------------------##

## 1.1.- Reseteo de variables.
%reset -f
## 1.2.- Importa las librerias necesarias.
import os
import subprocess
import commands
import numpy as np
...
## 2.- Definicion de directorios y nombres basicos.
# Nombre de imagen .jpg.
nombre_img_jpg = '26_07_-_20_41_51_image7_800.jpg'
# Directorio imagen .jpg.
dir_img_jpg = '/home/samuel/Desktop/SPEL/Star_Tracker/Mediciones/\
Prueba_sabado_06-08-16/' + nombre_img_jpg
# Directorio donde se guarda imagen .fits.
dir_img_fits = '/home/samuel/Desktop/Star_Tracker/Mediciones/scripts/'
# Nombre imagen .fits.
nombre_img_fits = 'img_fits.fits'
# Directorio SExtractor.
dir_sext = '/home/samuel/Desktop/Star_Tracker/Programa_completo/sextractor'
# Directorio base del catalogo proyectado.
path_base = '/home/samuel/Desktop/Star_Tracker/Division_de_catalogo/\
Scripts/Catalogo_dividido_y_proyectado/RA_10_DEC_10_V2/Proyectado/'
# Directorio donde se guarda el archivo 'sext'.
path_stars = dir_img_fits + 'sext'
# Directorio base de catalogo no proyectado.
cat_no_proy = '/home/samuel/Desktop/Star_Tracker/Division_de_catalogo/\
Scripts/Catalogo_dividido_y_proyectado/RA_10_DEC_10_V2/Normal/'
...
## 3.- Toma la imagen y transforma a .fits.
# Lectura de imagen.
...
ascii.write([sex_x1, sex_y1, sex_mag], 'sext', delimiter = ' ', format = 'no_header',\
            formats = {'col0':'% 15.10f', 'col1':'% 15.10f', 'col2':'% 15.10f'})

## 5.- Se hace 'match' con cada parte del catalogo de 10 deg de separacion.
...
for j in range (-90, 100, 180):
    # Define intervalo de DEC (en -90° y 90°).
    DEC2 = j
    # Transforma en string RA y DEC y arma el path del catalogo.
...
if len(match1_tabla1) == 0:
    ...
    # Coordenadas de centro de foto en mm.
    match1_x_pix = 0
    match1_y_pix = 0
...
## 6.- Deproyeccion del punto (0, 0) de la camara.
f = 3.04 #mm
...
## 7.- Busqueda y lectura de objetos de catalogo con los que se hizo match.
...
## 8.- Conversion de coordenadas de estrellas de match a plano tangente.
...
ascii.write(cat_tran1, 'new_cat', delimiter = ' ', format = 'no_header',\
            formats = {'col0':'% 15.5f', 'col1':'% 15.5f', 'col2':'% 15.2f'})

## 9.- Se hace el nuevo match.
...
## 10.- Nueva deproyeccion del punto (0, 0) de la camara.
...
## 11.- Nueva proyeccion en nuevo punto central encontrado.
## Conversion de coordenadas de estrellas de match a plano tangente.
# Iniciamos arrays vacios.
cat_tran2 = Table([[], [], []])
...
# Guarda el nuevo catalogo.
ascii.write(cat_tran2, 'new_cat', delimiter = ' ', format = 'no_header',\
            formats = {'col0':'% 15.5f', 'col1':'% 15.5f', 'col2':'% 15.2f'})

## 12.- Se hace el nuevo match.
...
## 13.- Nueva deproyeccion del punto (0, 0) de la camara.
...
## 14.- Resultados.
print '-'*20
print 'Los resultados son (en grados):'
